...
WebSocket 不是萬能的，有些場景輪詢反而更適合：

| 選輪詢 | 選 WebSocket |
|--------|--------------|
| 更新頻率低（幾分鐘一次） | 更新頻率高（秒級、毫秒級） |
| 不需要即時，晚幾秒沒差 | 要即時，延遲會影響體驗 |
| 只有前端問後端 | 需要雙向溝通 |
| 不想處理連線管理 | 願意處理斷線重連 |
| 伺服器不支援 WebSocket | 伺服器支援長連線 |

**我自己的判斷：**

- 檢查有沒有新郵件 → **輪詢**，每分鐘問一次就好
- 檢查有沒有新通知 → **看情況**，不急的話輪詢，要即時就 WebSocket
- 聊天訊息 → **WebSocket**，等三秒才看到訊息會很怪
- 下載進度 → **WebSocket**，進度條要順

輪詢的好處是**簡單**——不用管連線狀態、不用處理斷線重連、後端也不用維護連線池。
