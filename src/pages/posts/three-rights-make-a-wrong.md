...
詳情見[LLM API 批量請求的正確姿勢](./llm-api-batch-request)。
...
**設計的時候，問「如果 X 變成 100 倍會怎樣」。** 很多組合在小規模的時候完全正常，放大之後才會爆。N+1 問題在本機測不出來、快取不一致在單機測不出來、API rate limit 在資料少的時候測不出來。所以要在腦中先放大，或者真的寫壓力測試。
